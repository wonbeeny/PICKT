...
# - author : yxonic
# - date : 2025-06-23
# - link : https://github.com/yxonic/DTransformer/blob/main/DTransformer/model.py
...
from .modeling_outputs import KTModelOutput as DTransformerModelOutput

MIN_SEQ_LEN = 5


class DTransformerMilktModel(nn.Module):
    def __init__(self, config):
        ...
        n_pid = config.num_question
        n_questions = config.num_concept
        self.n_questions = config.num_concept
        self.q_embed = nn.Embedding(n_questions + 1, d_model)
        self.s_embed = nn.Embedding(2, d_model)
        self.pad_id = config.response2id["pad_id"]
    ...
    def get_loss(self, q, s, pid=None, label=None):
        logits, _, _, reg_loss, _ = self.predict(q, s, pid)
        seq_mask = label != self.pad_id
        # masked_labels = s[s >= 0].float()
        # masked_logits = logits[s >= 0]
        masked_labels = label[label < 2].float()
        masked_logits = logits[label < 2]
        return logits, (
    ...
    def get_cl_loss(self, q, s, pid=None, label=None):
        bs = s.size(0)

        # skip CL for batches that are too short
        # lens = (s >= 0).sum(dim=1)
        lens = (label < 2).sum(dim=1)
        minlen = lens.min().item()
        if minlen < MIN_SEQ_LEN:
            logits, loss = self.get_loss(q, s, pid, label)
            return DTransformerModelOutput(
                logits=loss,
        ...
        for i in range(1, self.window):
            ...
            pred_loss += F.binary_cross_entropy_with_logits(
                y[labels < 2], labels[labels < 2].float()
            )
        ...
        return DTransformerModelOutput(
            logits=pred_loss + cl_loss * self.lambda_cl + reg_loss,
            predictions=torch.sigmoid(logits),
            encoder_output=None,
            decoder_output=None
        )
...
class DTransformerLayer(nn.Module):
    ...
    def forward(self, query, key, values, lens, peek_cur=False):
        ...
        mask = mask.bool()[None, None, :, :].to(query.device)
