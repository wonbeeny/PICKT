...
parser.add_argument('--questions_path', default='', help="Question 파일 경로")
...
def data_load(args) -> pd.DataFrame():
    ...
    question_diff = {}
    for question_id in question['id'].unique():
        difficulty = question[question['id'] == question_id]['difficulty'].item()
        
        if difficulty == 1:
            question_diff[int(question_id)] = str(difficulty)
        elif difficulty == 2:
            question_diff[int(question_id)] = str(difficulty)
        elif difficulty == 3:
            question_diff[int(question_id)] = str(difficulty)
    ...
    log = log[~log.duplicated(subset=['student_id','question_id','start_time'])]
    ...
    log['difficulty'] = log['question_id'].map(question_diff)
    ...
    log = log[['student_id', 'new_start_time', 'new_end_time', 'answer_state', 'type', 'questionid_encoded', 'concept_ids', 'lag_time', 'elapsed_time', 'difficulty']]
    print(log.shape)
    print(log['student_id'].unique().shape)
    print(log['answer_state'].value_counts())
...
def preprocess_data(data: pd.DataFrame, data_args: dict):
    ...
    data["difficulty_ids"]   = data["difficulty"].map(data_args["difficulty2id"]).fillna(data_args["difficulty2id"]["unk_id"]).astype(int)
