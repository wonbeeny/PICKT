...
parser.add_argument('--preprocessed_data_path', default="/home/jovyan/work/PICKT/data/Online/preprocessed_data.parquet", help='전처리되어 저장된 데이터의 경로')
parser.add_argument('--datasets_save_path', default="/home/jovyan/work/PICKT/data/Online/", help='train/valid/test 데이터셋 저장 경로')
...
def mk_datasets(preprocessed_data, user_ids):
    ...
    for uid in user_ids:
        ...
        encoder_inputs["content_ids"].append(preprocessed_data[uid]["content_ids"])
        encoder_inputs["activity_ids"].append(preprocessed_data[uid]["activity_ids"])
...
def save_json(args, save_name, data):

    save_path = os.path.join(args.datasets_save_path, save_name)
...
if __name__ == "__main__":
    ...
    for user_id in tqdm(df.columns):
        ...
        preprocessed_data[user_id] = {
            ...
            "content_ids": user_data["content_ids"],
            "activity_ids": user_data["activity_ids"],
